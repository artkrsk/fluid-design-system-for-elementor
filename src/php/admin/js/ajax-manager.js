...
(($) => {
		...
		$.ajax({
			...
			success: (response) => {
				...
				// Update tables with data if provided (for individual operations)
					...
					response.data &&
					(response.data.main_groups || response.data.filter_groups)
				) {
				...
				if (response.success) {
					...
					if (
						...
						const message =
							response.message ||
							fluidDesignSystemAdmin.strings.operationSuccess;
					...
					if (typeof errorCallback === "function") {
						...
						if (
							...
							const message =
								response.message ||
								fluidDesignSystemAdmin.strings.operationError;
			...
			error: (xhr, status, error) => {
				...
				if (
					...
					const message =
						fluidDesignSystemAdmin.strings.ajaxError ||
	...
	function reorderGroups(groupOrder, successCallback, errorCallback) {
		...
		const hasTemporaryGroups = groupOrder.some((id) =>
	...
	function showStatus(type, message, options = {}) {
		if (
			...
			return window.FluidDesignSystemAdmin.statusNotices.showStatus(
				type,
				message,
				options,
			);
	...
	function showValidationFeedback(scenario, data = {}) {
		if (
			...
			return window.FluidDesignSystemAdmin.statusNotices.showValidationFeedback(
				scenario,
				data,
			);
		}
