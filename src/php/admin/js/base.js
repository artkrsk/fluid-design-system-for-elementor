...
(($) => {
	...
	function initSaveChangesAjax() {
		...
		$(document).on(
			"click",
			'#fluid-groups-form input[name="save_changes"]',
			function (e) {
				e.preventDefault();
				...
				if (
					...
					$("body").hasClass("ajax-operation-pending")
				) {
					return;
				}
	...
	function unmarkRowForDeletion($row) {
		...
		$(
			`#fluid-groups-form input[name="delete_groups[]"][value="${groupId}"]`,
		).remove();
	...
	function initInlineAddGroup() {
		...
		$(document).on("keydown", "#inline-add-row .add-title-input", (e) => {
		...
		$(document).on("keydown", "#inline-add-row .add-description-input", (e) => {
			if (e.key === "Enter") {
				e.preventDefault();
				saveInlineAdd();
			} else if (e.key === "Escape") {
		...
		$(document).on(
			"input",
			"#inline-add-row .add-description-input",
			function () {
				const $addRow = $("#inline-add-row");
		...
		$(document).on(
			...
			() => {
				...
				setTimeout(() => {
					...
					const isWithinAddRow =
						$focusedElement.closest("#inline-add-row").length > 0;

					if (!isWithinAddRow && $addRow.hasClass("editing")) {
						// Check if both fields are empty
						const titleValue = $addRow.find(".add-title-input").val().trim();
						const descriptionValue = $addRow
							.find(".add-description-input")
							.val()
							.trim();
		...
		$(document).on("click", "#inline-add-row .add-group-save", (e) => {
			e.preventDefault();
			e.stopPropagation();
			saveInlineAdd();
		});

		$(document).on("click", "#inline-add-row .add-group-cancel", (e) => {
			e.preventDefault();
			e.stopPropagation();
			cancelInlineAdd();
		});
		...
		const $measurer = $("<span>")
			.css({
				...
				fontSize: $textElement
					? $textElement.css("fontSize")
					: $input.css("fontSize"),
				fontFamily: $textElement
					? $textElement.css("fontFamily")
					: $input.css("fontFamily"),
				fontWeight: $textElement
					? $textElement.css("fontWeight")
					: $input.css("fontWeight"),
	...
	function saveInlineAdd() {
		...
		convertAddRowToRegular(
			$addRow,
			sanitizedGroupName,
			sanitizedGroupDescription,
		);
	...
	function convertAddRowToRegular($addRow, groupName, groupDescription) {
		...
		const escapedGroupDescriptionAttr = (groupDescription || "").replace(
			/"/g,
			"&quot;",
		);
	...
	function updateFormDataFromTempGroups() {
		...
		$("#fluid-groups-tbody tr.sortable-row").each(function (index) {
			...
			if ($row.data("temp-group")) {
				...
				const description =
					$row.find('input[name^="group_descriptions"]').val() || "";
