...
(($) => {
	...
	function showStatus(type, message, options = {}) {
		...
		$statusArea
			.removeClass("hidden loading success is-error warning info")
			.addClass(cssClass);
	...
	function createStatusArea() {
		...
		if ($container.length === 0) {
			...
			const $fallback = $(
				"#fluid-groups-form, .fluid-design-system-form-page",
			).first();
	...
	function showValidationFeedback(scenario, data = {}) {
		...
		switch (scenario) {
			...
			case "duplicateDetected":
				message = replacePlaceholders(strings.duplicateTitle, {
					name: data.name,
				});
				break;
			case "emptyTitle":
			...
			case "tooShort":
				...
				break;
			case "addingSuccess":
				message = replacePlaceholders(strings.addingSuccess, {
					name: data.name,
				});
				break;
			case "updatingSuccess":
				message = replacePlaceholders(strings.updatingSuccess, {
					name: data.name,
				});
				break;
			case "savingChanges":
