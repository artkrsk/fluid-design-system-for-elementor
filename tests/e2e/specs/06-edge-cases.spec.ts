...
test.describe('Inverted Min/Max (min > max)', () => {
  ...
  test('clamps correctly when min > max at small viewport', async ({ page }) => {
    await page.setViewportSize(TEST_VIEWPORTS.mobile)

    const heading = page.locator(getHeadingTitleSelector(TEST_ELEMENT_IDS.headingInverted))
  ...
  test('clamps correctly when min > max at large viewport', async ({ page }) => {
  ...
  test('values decrease as viewport increases (inverted scaling)', async ({ page }) => {
    const heading = page.locator(getHeadingTitleSelector(TEST_ELEMENT_IDS.headingInverted))
    ...
    // At small viewport
    await page.setViewportSize(TEST_VIEWPORTS.mobile)
    const smallValue = await heading.evaluate((el) => parseFloat(getComputedStyle(el).fontSize))
    ...
    const largeValue = await heading.evaluate((el) => parseFloat(getComputedStyle(el).fontSize))
...
test.describe('Custom Breakpoints', () => {
  ...
  test('respects custom min breakpoint', async ({ page }) => {
    ...
    const heading = page.locator(getHeadingTitleSelector(TEST_ELEMENT_IDS.customBreakpoint))
    await expect(heading).toBeVisible()

    const fontSize = await heading.evaluate((el) => parseFloat(getComputedStyle(el).fontSize))

    // At custom min (400px), value should be 10px
    expect(fontSize).toBeCloseTo(getExpectedValue('e2e_custom_breakpoints', 400), VALUE_TOLERANCE)
  })
  ...
  test('respects custom max breakpoint', async ({ page }) => {
    ...
    expect(fontSize).toBeCloseTo(getExpectedValue('e2e_custom_breakpoints', 1600), VALUE_TOLERANCE)
  })
  ...
  test('interpolates within custom range', async ({ page }) => {
    ...
    expect(fontSize).toBeCloseTo(getExpectedValue('e2e_custom_breakpoints', 1000), VALUE_TOLERANCE)
  })
  ...
  test('clamps to min value below custom min breakpoint', async ({ page }) => {
    ...
    expect(fontSize).toBeCloseTo(getExpectedValue('e2e_custom_breakpoints', 360), VALUE_TOLERANCE)
  })
  ...
  test('clamps to max value above custom max breakpoint', async ({ page }) => {
    ...
    expect(fontSize).toBeCloseTo(getExpectedValue('e2e_custom_breakpoints', 1920), VALUE_TOLERANCE)
  })
})
  ...
  test('all test CSS variables are defined in :root', async ({ page }) => {
    ...
    const cssVars = await page.evaluate(() => {
      ...
      return {
        ...
        customBreakpoints: style.getPropertyValue('--arts-fluid-preset--e2e_custom_breakpoints')
      }
    })
...
test.describe('CSS Formula Structure', () => {
  ...
  test('e2e_heading_xl formula contains correct min/max values', async ({ page }) => {
  ...
  test('e2e_negative_margin formula contains negative values', async ({ page }) => {
    const cssVar = await page.evaluate(() => {
      return getComputedStyle(document.documentElement).getPropertyValue(
  ...
  test('e2e_custom_breakpoints formula uses custom screen widths', async ({ page }) => {
    const cssVar = await page.evaluate(() => {
      return getComputedStyle(document.documentElement).getPropertyValue(
  ...
  test('e2e_static_value formula simplifies when min equals max', async ({ page }) => {
    const cssVar = await page.evaluate(() => {
      return getComputedStyle(document.documentElement).getPropertyValue(
...
test.describe('Compound CSS Properties (Issue #23)', () => {
  ...
  test('background-position renders without fluid keyword', async ({ page }) => {
    ...
    const bgPosition = await el.evaluate((el) => getComputedStyle(el).backgroundPosition)
